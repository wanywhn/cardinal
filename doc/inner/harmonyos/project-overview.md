...
- **Frontend (ArkUI/ArkTS)**: HarmonyOS ArkUI interface built with declarative UI syntax. Communicates with Rust backend via FFI through ohos-rs toolkit. Handles search UI, results display, file operations, and system integration.
- **Rust FFI bridge**: Uses ohos-rs (OpenHarmony Rust toolkit) to expose Rust backend functionality to ArkTS. Implements command handlers for search, metadata retrieval, file operations, and system services.
...
└─────────────────▲───────────────────────────────────────────────┘
                  │ ohos-rs Task-based communication
                  │ (search_task, node_info_callback, icon_prefetch,
                  │  rescan_handler, config_watcher, ...)
                  │
┌─────────────────┴───────────────────────────────────────────────┐
...
1. **Initialization**: ArkUI app initializes and calls into Rust FFI layer via ohos-rs. Rust backend loads persistent cache when possible (`SearchCache::try_read_persistent_cache`); otherwise walks the filesystem via `build_search_cache`.
...
- **ohos-rs toolkit**: Utilizes the ohos-rs (OpenHarmony Rust) toolkit to facilitate Rust integration with HarmonyOS applications. Provides standardized interfaces between Rust code and ArkTS.
...
- `harmony-bindings/`: FFI layer using ohos-rs to expose Rust functionality to ArkTS. Contains type mappings and async task management.
...
- **HarmonyOS**: Use DevEco Studio for ArkUI development; integrate with Rust components via ohos-rs toolkit. Build process includes Rust compilation for HarmonyOS targets using NDK.
